...
The wavefront simulation of the beamline include propagation of the beam from element to element (drift spaces) using different possible propagators. A propagator based on the Freskel-Kirchhoff integral is used. The applications of this (and other) operators imply the calculation of an integral (or sum) at each pixel of the image plane. This is costly from the computer point of view, so Fourier Transforms (Fourier Optics) are used to reduce complexity and calculation time. This is practically the only choice for simulating 2D images. The use of Fourier Transforms adds another problem: the result is very dependent on the sampling of the wavefront (i.e., pixel size, number of points, dimension of the window where the wavefront is sample). This means the usually one has to test many configurations before finding one that works reaonably well (even though one can never guarantee the complete accuracy of the sampling used). A full wavefront optics simulation is an iterative process, where the user has to refine the sampling parameters element by element to be sure the wavefront at each position is sampled correctly. This is a time consuming work for performing it with the full 2D simulation. For this reason, we found very useful to start a wavefront calculation in 1D, separating the horizontal and vertical simulations, and using a simplified system with ideal sources and ideal focusing elements. This is ofted a good choice because the horizontal and vertical coupling is usually small in synchrotron beamlines. This 1D simulations help in refining the choice of propagator and its parameters (resampling, zoom factors) that will be valid if a, more costly, full 2D simulation is envisaged.

Many of the simulations presented here (ray tracing and coherent optics in Sections XXXXX) can be done interactively in a normal laptop. However, when one one to obtain information on the partial coherence, very long calculations have to be done. For example, Monte Carlo calculations in Section~\ref{srw_me} took significantly longer to be completed for the multi electron case: a few hours on a 56-core computer cluster. For performing the coherent mode decomposition the calculation time depends very much on the dimensioning on the problem. The EBS case is reasonable: we sampled the wavefronts using 1007x335 pixels and calculated 1103 modes that represent 98\% of the spectral density. This represents 5 MB per wavefront and a matrix of 1695 GB to diagonalize. It took less that four hours in the a cluster of 28 cores. The calculation is more expensive if one wants to decompose a source that is pretty incoherent like our ESRF-High-$\beta$. This is a limit case, as the wavefields have to be vey large XXxXX to accomodate the large espatial extension of the source. The calculation presented here took XX days in a large XX cluster. To give an idea of the real cost, a similar work send to the AWS cluster costed about 2000 USD. This was done as a test case, to evaluate the limits of the calculations, but it is certainly not recommended to do similar things, not only for the cost but also for the lack of interest to treat with partial coherence methods a source that is quite incoherent. In Appendix B we present for the ESRF High$\beta$ similar results than those in Section~\ref{comsyl}. 

There are some effects that are typically studied when designing and simulating a beamline that have been ignores here. For example the study of the power emitted by the source and its heat load effect in the first (or firsts) slits and optical elements (here the ML monochromator). Also reflectivity of the elements are not considered. Thir effect can also be fully simulated, but we preferred here to integrate them in the simulations for the sake of clarity. Again an analytical estimation would give a reflectivity about 0.7 for multilayers and 0.9 for mirrors, therefore a reduction factor of (0.7)**3 ML for the beamline flux due to reflectivity of mirror coatings. 

In conclusion, we have shown how to obtain the relevant parameters necessary to quantify flux, transmission, fucused beam size, and coherent fraction for a modern beamline with already exceptional performances in the old ESRF DBA lattices, and advanced the improvements to be expected when the EBS will start in 2020. In this discussion we obtained these parameters using a hierarchical flowchart, with increasing complexity. The phase of analytical estimation of the main parameters is essential, not only to start with some values but also because these values help to perform sanity check of the computer simulations. A ray tracing simulation is inexpensive and always recommended. The comparison of the CF beamline transmittivity would give an idea of how coherent is the beam, or how many modes will be present in your final beam. For high coherence (a few modes), like for ID16A, the wave optics calculations are necessary. If the final beam has a very high coherence, like in our case, coherent optics (1D simplifications, and single electron calculations) may give good results. We may have stopped there, as in our case coherence is very high. The COMSYL calculations presented here introduce a new way of quantitatively assess the quality of the partial coherent beam via the calculation of the coherent fraction.  
