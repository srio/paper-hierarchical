...
%\input{tableUndulator.txt}
 ESRF-1 U18.3  & 18.30  & 1.40  & 2 & 1.175(n=1) & 0.470(n=1) & 1.175(n=3)\\ 
 ESRF-1 U22.4  & 22.40  & 1.40  & 2 & 0.873(n=1) & 1.855(n=3) & 0.873(n=3)\\ \\
 EBS U18.3  & 18.30  & 1.40  & 2 & 1.156(n=1) & 0.437(n=1) & 1.156(n=3)\\ 
 EBS U22.4  & 22.40  & 1.40  & 2 & 0.852(n=1) & 1.836(n=3) & 0.852(n=3)\\ 
\end{tabular}%
}
\end{table}
...
\begin{table}\label{tab:eBeam}
    \centering
    \caption{ESRF-1 and EBS main electron beam parameters. Electron beam parameters for the high-$\beta$ (labelled ESRF) and the EBS straight sections. Values taken at the symmetry point of the straight section, where the insertion devices are placed. Values are taken from \cite{orangebook}, and updated for EBS with
    the lattice labelled S28D. 
    %For EBS, two (very close) design values are used, from Ref.~\cite{ESRF2014} labelled EBS-S28A, and those coming from further lattice refinement, labelled EBS-S28D. 
    %\todo{Remove ESRF-AT and EBS-S28A} 
    }
        \resizebox{\textwidth}{!}{%
        \begin{tabular}{ccccc}
         ...
         %ESRF-AT  & 414.97  & 3.43  & 10.31 & 1.16 \\ 
         %EBS-S28A  & 27.20  & 3.40  & 5.20 & 1.40 \\ 
...
\begin{table}\label{tab:HandCalculations}
    ...
    %(21.3 if approximating by the Gaussian $\sigma$).
    }
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{r|ccc|ccc}
                            &               & \textbf{ESRF-1 (high-$\beta$)} &   &         & \textbf{EBS}  &    \\ 
...
%($\max(\Sigma') \times d = 0.73$~mm), with $\Sigma'$ values from in Table~\ref{tab:HandCalculations}. 
...
%Although very powerful commercial ray-tracing packages are available in the market, they are generally ill-adapted for synchrotron simulations because they do not cover the particularities of X-ray devices and sources. For that purposes, 
...
%One of the difficulties with wave-optics simulations is the correct sampling and range of the input wavefront. If chosen poorly, the simulation will be affected by replicas and aliasing, compromising, then the quality of the obtained data. Choosing the range of the wavefront is straightforward: one can base the choice either on the first optical element aperture or on the beam footprint at its entrance. Usually one chooses the smaller. Choosing a very small range for the wavefront leads to cropping and diffraction effects that may not come from the beamline, but only a numerical consequence of truncating the wavefront. For the ID16A, the first optical element is the multi-layer monochromator and its aperture accepts the full beam of both the ESRF-1 and EBS lattices. 
%Choosing the correct sampling is a more difficult task: good sampling implies choosing an appropriate number of pixels such as the phase of the electric field being propagated varies less than $\pi$ between two adjacent pixels. This rule may lead to very big arrays: SRW keeps in memory both $\sigma-$ and $\pi-$polarizations. Doubling the number of pixels on both directions of a 2D array implies an increase in memory consumption of about 8 times.  

%All simulations using SRW were done using the same grid ($3.5\times1.5$mm$^2$) and initial sampling ($740\times150$) - the initial wavefront was resampled by a factor of 8 in order to mitigate aliasing and replicas throughout the beamline.
...
% Table \ref{tab:SRW_results} compiles the beam sizes at focal position for mentioned simulations.
...
the same, with errors             & Horizontal &  9.36/10.61      & 13.20/12.52 & 17.78/15.71 \\
ideal lenses/ elliptical mirrors  & Vertical   &  7.22/7.94       & 8.66/9.38   & 8.90/10.11    \\
the same, with errors             & Vertical   &  7.70/8.90       & 8.42/9.14   & 8.90/9.86    \\                                            
\end{tabular}
}
\end{table}
...
\includegraphics[width=0.9\textwidth]{GRAPHICS/metrology.eps}
